...
def startpage():
    ...
    print("Welcome to " + bold("Nutella Market 3.0"))
    ...
    if n == "1":
        ...
        print("\nAnd in the " + bold("mode 1") + " only,")
        print("The maximum price of a nutella is $450", )
        print("The maximum working effiency of a nutella is $7.87/s, ")
        print("while in other mode, there is no upper limit of them!")
...
def gametick():
    global private, pubnum, money, mode, start, stop, workspeed, timea, timeb, gamemode
    if gamemode == 1:
        if money >= 1000:
            endpage()
        elif money < 0:
    ...
    for i in range(len(private)):
        if private[i][3] <= 15:
            ...
            print(bold("NEWS!!!"))
            print("nutella " + str(private[i][0]) + " has a loyalty that was too low, so it escape from your farm. ")
            print("Other nutella spetator his escaption, their working effiency and loyalty decrease. ")
            private.pop(i)
            for j in range(len(private)):
                private[j][3] *= rd(0.8,0.9)
                private[j][2] *= rd(0.8,0.9)
            line()
            break
    workspeed = 0
    for i in range(len(private)):
...
def buy():
    ...
    while True:
        ...
        for i in range(randint(1,3)):
            ...
            if money <= 2:
                p = rd(2,5)
                public.append([pubnum,p,(rd(p/20 - p/50, p/20 + p/50))*0.6, rd(40,60)])
            elif money <= 10:
                p = rd(2, money*0.95)
                public.append([pubnum,p,(rd(p/20 - p/50, p/20 + p/50))*0.5, rd(40,60)])
            elif money <= 200:
                p = rd(money*0.5, money*0.8)
                public.append([pubnum,p,(rd(p/20 - p/50, p/20 + p/50))*0.4, rd(40,60)])
            elif money <= 500:
                p = rd(money*0.4, money*0.6)
                public.append([pubnum,p,(rd(p/20 - p/50, p/20 + p/50))*0.3, rd(40,60)])
            elif money > 500:
                p = rd(money*0.3, money*0.45)
                public.append([pubnum,p,(rd(p/20 - p/50, p/20 + p/50))*0.25, rd(40,60)])
                ...
                if n != "": #make sure the player enter something
                    for i in range(0,len(private)): #check is the nutella exist
                        if private[i][0] == int(n): #if the nutella exist
                            k = True
                            line()
                            rewardmoney = private[i][1] * rd(0.05,0.2)
                            print("Are you sure to reward nutella " + str(private[i][0]) + " using $" + dp(str(rewardmoney)) + "?")
...
#define the checking function (mode 5)
def check():
    global private, public, pubnum, money, mode, start, stop, workspeed, timea, timeb, gamemode
    while True:
        clearscreen()
